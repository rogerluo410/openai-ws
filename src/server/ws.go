...
func (w *WsConn) Reader(client *Client, processor *Processor) {
	defer func() {
		log.Info("Reader 退出...")
	...
	for {
		...
		// 写入管道
		l.Info("客户端发送数据!")
		job := &Job{Client: client, Type: 0, Message: string(msg)}
		processor.SendMsg <- job
	}
}

func (w *WsConn) Writer(client *Client, processor *Processor) {
